...
export default function EmployeeProfilePage() {
  ...
  const fetchProfile = async () => {
    ...
    try {
      ...
      setFetchError(err.message || 'Gagal memuat profil karyawan');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleBankSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setBankSuccess('Nomor rekening bank berhasil diperbarui!');
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
        ...
        <p className="text-sm text-slate-400">Memuat profil karyawan...</p>
      </div>
    );
  }
    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <div>
        ...
        <p className="text-slate-400 mt-1 text-sm">
          Kelola foto profil, informasi diri, nomor rekening penggajian, dan ubah kata sandi akun Anda.
        </p>
      </div>

      {profileSuccess && (
        <div className="p-4 bg-emerald-950/40 border border-emerald-800/60 text-emerald-300 rounded-2xl text-xs flex items-center gap-2.5 animate-fadeIn">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 relative overflow-hidden shadow-xl">
            ...
            <div className="flex flex-col items-center text-center space-y-4 pb-6 border-b border-slate-800">
              {/* Profile Photo Avatar (Entire Circle Clickable to Upload) */}
              <label
              ...
              >
                <div className="w-28 h-28 rounded-full bg-gradient-to-br from-violet-600 to-indigo-600 flex items-center justify-center text-4xl font-black text-white shadow-xl shadow-violet-950/50 border-4 border-slate-800 transition-all duration-300 group-hover:border-violet-500 overflow-hidden relative">
                  {photo ? (
                    ...
                    <span className="transition-all duration-300 group-hover:scale-110">{profile?.name ? profile.name.charAt(0).toUpperCase() : 'U'}</span>
                  )}
              ...
              <div className="w-full">
                {editingName ? (
                  <form onSubmit={handleNameSubmit} className="mt-1 flex items-center gap-2">
                    <input
                      ...
                      className="w-full px-3 py-1.5 bg-slate-950 border border-slate-800 rounded-xl text-slate-200 text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-violet-500/50 text-center"
                    />
                    <button
                      type="submit"
                      disabled={updatingName}
                      className="p-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl transition cursor-pointer shrink-0"
                      title="Simpan Nama"
                    >
                    ...
                    <button
                      ...
                      className="p-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl text-xs font-semibold transition cursor-pointer shrink-0"
                    >
                      Batal
                    </button>
                  </form>
                  ...
                  <div className="flex items-center justify-center gap-2 group">
                    ...
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 p-1 text-slate-400 hover:text-violet-400 transition cursor-pointer"
                      title="Ubah Nama"
                    >
                ...
                <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-violet-600/20 border border-violet-500/30 text-violet-300 rounded-full text-xs font-semibold mt-2">
            ...
            <div className="pt-6 space-y-4 text-xs">
              ...
              <div className="flex items-center justify-between py-2 border-b border-slate-800/60">
                <span className="text-slate-400 flex items-center gap-2">
                  <Building2 className="w-4 h-4 text-slate-500" /> Divisi / Departemen
                </span>
              ...
              <div className="flex items-center justify-between py-2 border-b border-slate-800/60">
                <span className="text-slate-400 flex items-center gap-2">
                  <User className="w-4 h-4 text-slate-500" /> Status PTKP
                </span>
                <span className="font-semibold text-slate-200">{profile?.ptkpStatus || 'TK/0'}</span>
              </div>

              <div className="flex items-center justify-between py-2 border-b border-slate-800/60">
                <span className="text-slate-400 flex items-center gap-2">
                  <Calendar className="w-4 h-4 text-slate-500" /> Tanggal Bergabung
                </span>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
            ...
            {bankSuccess && (
              <div className="mt-3 p-3 bg-emerald-950/40 border border-emerald-800/60 text-emerald-300 rounded-xl text-xs flex items-center gap-2">
        ...
        <div className="lg:col-span-2">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl relative overflow-hidden">
            <div className="flex items-center gap-3 pb-6 border-b border-slate-800">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-100">Ubah Kata Sandi Akun</h3>
                <p className="text-xs text-slate-400">
                  Perbarui kata sandi Anda secara berkala untuk menjaga keamanan data penggajian.
                </p>
              </div>
            </div>
